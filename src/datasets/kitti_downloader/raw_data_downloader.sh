...
#	echo "Full URL: http://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/$fullname"
#        #wget 'http://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
#        curl -o 'http://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
...
for i in ${files[@]}; do
    if [ ${i:(-3)} != "zip" ]; then
        shortname=$i'_sync.zip'
        fullname=$i'_sync.zip'
        url='http://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
    else
        shortname=$i
        fullname=$i
        url='http://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
    ...
    echo "Downloading: "$shortname
    echo "Full URL: $url"  # Debug output
    curl -L -o $shortname $url  # Follow redirects with -L
    ...
    if [ -s $shortname ]; then
        ...
        echo "Downloaded file $shortname is empty or invalid."
        rm $shortname
    fi
done